...
class GoodsSchema extends Schema {
  up () {
    this.create('ni_goods', (table) => {
      ...
      table.integer('goods_is_new', 1).comment('是否为新品')
      table.integer('goods_is_hot', 1).comment('是否为热销')
      table.integer('goods_is_best', 1).comment('是否为精品')
      table.integer('goods_is_promotions', 1).comment('是否正在促销')
      ...
      table.integer('goods_integral', 64).comment('商品积分')
      table.integer('goods_is_real', 1).comment('是否为虚拟商品')
      table.timestamps()
    })
  }
